...
【あなたの役割】
あなたは、不動産会社のデータ分析担当者です。会社では、物件の特徴が
似ているものを自動的にグループ分けし、お客様へおすすめする仕組みを
作ろうとしています。

【困っていること】
物件データには「家賃（円）」「広さ（平方メートル）」「駅徒歩（分）」
「築年数（年）」があります。しかし、家賃は数万から十数万という値、
ほかの項目は数十程度の値です。このまま機械学習を行うと、数値が大きい
家賃ばかりが強く影響する可能性があります。
