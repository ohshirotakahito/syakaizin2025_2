# -*- coding: utf-8 -*-
"""
演習：食品包装材の溶出試験を想定した質量スペクトル解析

【想定する場面】
食品包装材の溶出試験で得た質量スペクトルから、ピークを自動検出し、
既知物質ライブラリと照合して候補物質を割り当てる。
"""
...
# TODO: scipy.signal.find_peaks()を使って、高さ150以上、
#       間隔10以上、prominence80以上のピークを検出してください
# ヒント： peak_indexes, properties = find_peaks(intensity, height=150, distance=10, prominence=80)
peak_indexes = None
...
def identify_compound(detected_mz):
    """検出したm/zに最も近いライブラリ化合物を、0.5以内なら候補として返す。"""
    # TODO: ここに実装してください
    # ヒント：
    #   differences = (library["reference_mz"] - detected_mz).abs()
    #   nearest_index = differences.idxmin()
    #   0.5以内なら library.loc[nearest_index, "compound"] を返す
    #   それ以外は "Unknown - confirmation required" を返す
    pass
